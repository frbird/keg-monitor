...
describe('Beers API (CRUD)', () => {
  ...
  describe('POST /api/admin/beers', () => {
    it('creates a beer and returns id', async () => {
      const name = `Test Beer ${Date.now()}`;
      const res = await agent
        .post('/api/admin/beers')
        .set('Content-Type', 'application/json')
        .send({ brewery: 'Test Brew', beer_style: 'IPA', name })
        .expect(201);
  ...
  describe('PUT /api/admin/beers/:id', () => {
    it('updates a beer', async () => {
      const name = `Beer ${Date.now()}`;
      const create = await agent.post('/api/admin/beers').set('Content-Type', 'application/json').send({ brewery: 'B', beer_style: 'S', name }).expect(201);
      const id = create.body.id;
      const updatedName = `Updated ${Date.now()}`;
      const res = await agent
        .put(`/api/admin/beers/${id}`)
        .set('Content-Type', 'application/json')
        .send({ brewery: 'Updated', beer_style: 'Lager', name: updatedName })
        .expect(200);
  ...
  describe('DELETE /api/admin/beers/:id', () => {
    it('deletes a beer', async () => {
      ...
      const create = await agent.post('/api/admin/beers').set('Content-Type', 'application/json').send({ brewery: 'Del', beer_style: 'S', name }).expect(201);
      const id = create.body.id;
